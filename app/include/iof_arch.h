...
void AirLinkResult( pgcontext pgc );

uint32 M2M_ReconnectTimer( pgcontext pgc );
uint32 Http_ReconnectTimer( pgcontext pgc );
...
int32 GAgent_DevGetConfigData( gconfig *pConfig );
int32 GAgent_DevSaveConfigData( gconfig *pConfig);
void GAgent_LocalDataIOInit( pgcontext pgc, int32 BaudRate );
uint32 GAgent_ReadOTAFile( uint32 offset, int8* buf,uint32 len,uint8 filetype );
...
int8 GAgent_DRVWiFiPowerScan( pgcontext pgc );
int8 GAgent_DRVWiFiPowerScanResult( pgcontext pgc );
int32 GAgent_StartUpgrade( void );
uint32 Http_ResGetFirmware( pgcontext pgc,int32 socketid,u16 length,uint8 otatype,uint8* httpReceiveBuf );
