...
make_rand(int8* data)
{
    ...
    data[10] = '\0';
    GAgent_Printf(GAGENT_CRITICAL,"passcode:%s",data );
}

// 将数字编码为可变长度
// 返回长度
int ICACHE_FLASH_ATTR
encodevarlen(int data, u8* buf)
{
    int l = 0;
    do
    {
...
void ICACHE_FLASH_ATTR
GAgent_String2MAC( uint8* string, uint8* mac )
{
...
GAgent_SetCheckSum(  uint8 *buf,int packLen )
{
    uint8 checkSum=0;
    int i;
...
GAgent_CheckSum( int8 *data,int dataLen )
{
    uint8 checksum=0;
    int i=0;
    for( i=0;i<dataLen;i++ )
...
***************************************************/
varc ICACHE_FLASH_ATTR
Tran2varc(uint32 remainLen)
{
    int i;
	varc Tmp;

    os_memset(&Tmp, 0x0, sizeof(Tmp));
...
SetPacketType( int32 currentType,int32 type,int8 flag )
{
    if( flag==0 )
    {
        currentType &=~ type;
...
/* return 1 type set */
int8 ICACHE_FLASH_ATTR
isPacketTypeSet( int32 currentType,int32 type )
{
...
ParsePacket( ppacket pRxBuf )
{
    int32 varlen=0;
    int32 datalen=0;
    uint16 cmd=0;
    uint16 *pcmd=NULL;
    pRxBuf->type = SetPacketType( pRxBuf->type,CLOUD_DATA_OUT,0 );
    pRxBuf->type = SetPacketType( pRxBuf->type,LAN_TCP_DATA_OUT,0 );
    pRxBuf->type = SetPacketType( pRxBuf->type,LOCAL_DATA_OUT,0 );
    if( ((pRxBuf->type)&(CLOUD_DATA_IN)) == CLOUD_DATA_IN )
    {
        ...
        cmd = pRxBuf->phead[4+varlen+1]<<8 | pRxBuf->phead[4+varlen+1+1];
...
//        cmd = HTONS(cmd);
        cmd = pRxBuf->phead[LAN_PROTOCOL_HEAD_LEN+varlen+LAN_PROTOCOL_FLAG_LEN]<<8 |
                pRxBuf->phead[LAN_PROTOCOL_HEAD_LEN+varlen+LAN_PROTOCOL_FLAG_LEN + 1];
...
int8 ICACHE_FLASH_ATTR
dealPacket( pgcontext pgc, ppacket pTxBuf )
{
...
//        GAgent_LocalDataWriteP0( pgc,pgc->rtinfo.local.uart_fd, pgc->rtinfo.Txbuf,MCU_CTRL_CMD );
    ...
    {
        GAgent_Printf( GAGENT_DEBUG,"packet Type : LAN_TCP_DATA_OUT ");
...
void ICACHE_FLASH_ATTR
copyPacket(ppacket psrcPacket, ppacket pdestPacket)
{
    ...
    os_memcpy(pdestPacket->phead, psrcPacket->phead, psrcPacket->pend - psrcPacket->phead);
}

void ICACHE_FLASH_ATTR
setChannelAttrs(pgcontext pgc, stCloudAttrs_t *cloudClient, stLanAttrs_t *lanClient, uint8 isBroadCast)
{
    if(isBroadCast)
    {
...
void ICACHE_FLASH_ATTR
clearChannelAttrs(pgcontext pgc)
{
