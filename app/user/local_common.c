...
parsefileinfo(pfdesc pf, ppacket pp)
{
...
/* 返回剩余包数量 */
int ICACHE_FLASH_ATTR
pushfiledata(pfdesc pf, ppacket pp)
{
    /* header, len, cmd, sn, flag,
...
/* 将文件发送到对端 */
int ICACHE_FLASH_ATTR
syncfile(pgcontext pgc, pfdesc pf)
{
    int ret;
    int i = 1;
...
int ICACHE_FLASH_ATTR
throwgarbage(pgcontext pgc, pfdesc pf)
{
    /*  */
    ...
    while(remain > 0)
    {
        ...
        GAgent_Printf(GAGENT_DEBUG, "SYNCFILE:throwgarbage  %d ytes == %x", remain, 0xa5 + i);
        i++;
#endif
        if(remain < 1024)
        {
        ...
        {
            GAgent_Printf(GAGENT_DEBUG, "SYNCFILE:throwgarbage send error");
            return RET_FAILED;
        }
        /* msleep(300); */
    }
...
resetfile(pfdesc pf)
{
    os_memset(pf, 0x0, sizeof(fdesc));
    return 0;
}

uint32 ICACHE_FLASH_ATTR
trans_parsemcuotainfo(pgcontext pgc, ppacket pp)
{
    int8 *p = NULL;
...
trans_parseotaurl(ppacket pp, u8 *url)
{
    int8 *p = NULL;
    int16 len = 0;
...
trans_dealmcutransction(pgcontext pgc, ppacket pp)
{
    u8 url[256] = {0};
...
int32 ICACHE_FLASH_ATTR
trans_sendotaresult(pgcontext pgc, u8 result)
{
    u8 *p = NULL;
...
/* 发送OTA固件下载结果 */
int32 ICACHE_FLASH_ATTR
trans_sendotadownloadresult(pgcontext pgc, u8 result)
{
    u8 *p = NULL;
    resetPacket(pgc->rtinfo.Txbuf);
    p = pgc->rtinfo.Txbuf->ppayload;
...
int32 ICACHE_FLASH_ATTR
trans_startmcuota(pgcontext pgc, u8 *url)
{
    int i = 0;
...
int32 ICACHE_FLASH_ATTR
trans_sendfirmwareinfo(pgcontext pgc, u8 *sv, u8 *url)
{
...
int32 ICACHE_FLASH_ATTR
trans_checkmcuota(pgcontext pgc, u8 *sv, u8 *url)
{
    int ret = 0;
    /* flag决定是否判断 */
    if(pgc->tmcu.check)
